...
- **Authentication**: Features `bcrypt` for secure password hashing and `express-session` with a PostgreSQL store for session management. Currently configured for **ADMIN-ONLY** login at `/admin/login`. Fixed admin credentials: `admin@ziba.app` / `ZibaAdmin2024!`. All admin routes are protected by AdminGuard requiring ADMIN role. User, Driver, and Director logins are disabled pending future implementation. Test account impersonation system allows admin to login as any test account for development testing.
- **Core Features**:
